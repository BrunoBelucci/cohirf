...
Adapted from https://github.com/IshankGulati/scikit-learn/blob/6ca0e6381e6a5afb5d14e16f2fa782b54b893a1e/sklearn/cluster/kernel_kmeans.py
"""
...
class KernelKMeans(BaseEstimator, ClusterMixin):
    ...
    * Inderjit S. Dhillon, Yuqiang Guan, Brian Kulis
    ...
    def _compute_dist(self, K, dist, within_distances, update_within):
        ...
        # VECTORIZED FORM (with some help of copilot)
        sw = np.array(self.sample_weight_)  # Sample weights (n_samples,)

        # Step 1: Create the one-hot encoded cluster assignment matrix
        Z = np.zeros((len(self.labels_), self.n_clusters))
        Z[np.arange(len(self.labels_)), self.labels_] = sw
        ...
        # we can ignore the first term as it is constant
